...
fn build_example(name: &str) {
    ...
    let tc_hard: Vec<_> = tc_errors
        ...
        .filter(|e| matches!(e.severity, Severity::Error))
        .collect();
    assert!(tc_hard.is_empty(), "type errors in {name}: {tc_hard:?}");
...
#[test]
fn build_hello() {
    build_example("hello.naze");
}

#[test]
fn build_boxes() {
    build_example("boxes.naze");
}

#[test]
fn build_columns() {
    build_example("columns.naze");
}

#[test]
fn build_rows() {
    build_example("rows.naze");
}

#[test]
fn build_nested() {
    build_example("nested.naze");
}

#[test]
fn build_padding() {
    build_example("padding.naze");
}

#[test]
fn build_rounded() {
    build_example("rounded.naze");
}

#[test]
fn build_colors() {
    build_example("colors.naze");
}

#[test]
fn build_typography() {
    build_example("typography.naze");
}

#[test]
fn build_grid() {
    build_example("grid.naze");
}

#[test]
fn build_dashboard_static() {
    build_example("dashboard-static.naze");
}

#[test]
fn build_app_shell() {
    build_example("app-shell.naze");
}
...
fn build_conditional() {
    build_example("conditional.naze");
}

// --- Examples with component imports ---

#[test]
fn build_component_basic() {
    build_example("component-basic.naze");
}

#[test]
fn build_component_props() {
    build_example("component-props.naze");
}

#[test]
fn build_multi_component() {
    build_example("multi-component.naze");
}
...
#[test]
fn build_theming() {
    build_example("theming.naze");
}

// --- Examples with layout features (Phase 2 M7) ---

#[test]
fn build_layout_features() {
    build_example("layout-features.naze");
}
